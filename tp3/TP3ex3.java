...
class TP3ex3{
    ...
    public static void insertionSort(Serie[] series, int numInput) throws Exception{
        for(int i = 0; i < numInput-1; i++){
            int min = i;

            for(int j = i+1; j < numInput; j++){
                if(series[j].getIdioma().compareTo(series[min].getIdioma()) < 0 ){
                    min = j;
                }else if(series[j].getIdioma().compareTo(series[min].getIdioma()) == 0 ){
    ...
    public static void main(String[] args) throws Exception{
        ...
        Arq.openWrite("matrícula_insercaoo.txt", "UTF-8");
