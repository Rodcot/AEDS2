...
class TP3ex8{
    ...
    public static void bubbleSort(Serie[] series, int numInput) throws Exception{
        for (int i = (numInput - 1); i > 0; i--) {
			for (int j = 0; j < i; j++) {
				if(series[j].getnTemporadas() > series[j+1].getnTemporadas()) {
                    Serie temp = series[j];
                    series[j] = series[j+1];
                    series[j+1] = temp;
                    comparacoes++;
                    movimentacoes++;
				}else if(series[j].getnTemporadas() == series[j+1].getnTemporadas()){
    ...
    public static void main(String[] args) throws Exception{
        ...
        Arq.openWrite("matrícula_bolha.txt", "UTF-8");
