...
void selectionSort(int nEntrada){
    for(int i = 0; i < nEntrada; i++){
        int min = i;

        for(int j = i+1; j < nEntrada; j++){
            if(strcmp(array[j].pais, array[min].pais) < 0){
                min = j;
            }else if(strcmp(array[j].pais, array[min].pais) == 0){
                if(strcmp(array[j].nome, array[min].nome) < 0){
                    min = j;
                }
            }
        }
        Serie temp = array[min];
        array[min] = array[i];
        array[i] = temp;
    }
}
