...
class TP3ex10{
    ...
    public static Serie getMax(Serie[] serie, int n){
        Serie mx = serie[0];
        for (int i = 1; i < n; i++){
            if ((serie[i].getnEpisodios()*1000+serie[i].getnTemporadas()) > ((mx.getnEpisodios()*1000)+mx.getnTemporadas())){
                mx = serie[i];
                comparacoes++;
                movimentacoes++;
            }
    ...
    // "((serie[i].getnEpisodios()*1000)+serie[i].getnTemporadas())" = VALOR DOIDÃO que a questão pede como chave de pesquisa, só me deu preguiça de criar uma váriavel pra ele.
    public static void countingSort(Serie[] serie, int n, int exp){
        Serie output[] = new Serie[n]; // output array
        int i;
        int count[] = new int[10];
 
        // Store count of occurrences in count[]
        for (i = 0; i < n; i++){
            count[(((serie[i].getnEpisodios()*1000)+serie[i].getnTemporadas()) / exp) % 10]++;
        }
        ...
        for (i = n - 1; i >= 0; i--) {
            output[count[(((serie[i].getnEpisodios()*1000)+serie[i].getnTemporadas()) / exp) % 10] - 1] = serie[i];
            count[(((serie[i].getnEpisodios()*1000)+serie[i].getnTemporadas()) / exp) % 10]--;
            movimentacoes++;
        }
 
        // Copy the output array to serie[], so that serie[] now
    ...
    public static void radixsort(Serie[] serie, int n){
        // Find the maximum number to know number of digits
        Serie m = getMax(serie, n);
 
        // Do counting sort for every digit. Note that
        // instead of passing digit number, exp is passed.
        // exp is 10^i where i is current digit number
        for (int exp = 1;((m.getnEpisodios()*1000)+m.getnTemporadas()) / exp > 0; exp *= 10){
            countingSort(serie, n, exp);
        }
    }
    ...
    public static void main(String[] args) throws Exception{
        ...
        //Pegando o array de series para fazer o insertion sort
        long inicio = now();
        radixsort(series, numInput);
        long fim = now();
        ...
        Arq.openWrite("matrícula_radixsort.txt", "UTF-8");
