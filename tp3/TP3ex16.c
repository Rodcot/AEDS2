...
void quickSort(int start, int end) {
        int i = start, j = end;
        Serie pivot = array[(start+end)/2];
        while(i <= j){
            while((strcmp(array[i].pais, pivot.pais) < 0) || ((strcmp(array[i].pais, pivot.pais) == 0) && (strcmp(array[i].nome, pivot.nome) < 0))){
                i++;
            }
            while((strcmp(array[j].pais, pivot.pais) > 0) || ((strcmp(array[j].pais, pivot.pais) == 0) && (strcmp(array[j].nome, pivot.nome) > 0))){
                j--;
            }
            if(i <= j){
                Serie temp = array[i];
