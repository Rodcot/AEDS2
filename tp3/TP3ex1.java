...
class TP3ex1{
    ...
    public static void selectionSort(Serie[] series, int numInput) throws Exception{
        for(int i = 0; i < numInput-1; i++){
            int min = i;

            for(int j = i+1; j < numInput; j++){
                if(series[j].getPaisOrigem().compareTo(series[min].getPaisOrigem()) < 0 ){
                    min = j;
                }else if(series[j].getPaisOrigem().compareTo(series[min].getPaisOrigem()) == 0 ){
    ...
    public static void main(String[] args) throws Exception{
        ...
        //Pegando o array de series para fazer o selection sort
        long inicio = now();
        selectionSort(series, numInput);
        long fim = now();
